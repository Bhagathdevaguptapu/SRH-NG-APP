...
public class EmployeeService {
	...
	public EmployeeDTO fetchEmployeeTicketsById(int id) {
		...
		if (entityOpt.isPresent()) {
			...
			List<TicketDTO> ticketDTOs = e.getTickets().stream().map(ticket -> new TicketDTO(ticket.getTicketId(),
					ticket.getTitle(), ticket.getDescription(), ticket.getStatus())).collect(Collectors.toList());
	...
	public void giveFeedback(int ticketId, String feedbackText) throws FeedbackAlreadyGivenException {
	    ...
	    if (ticketOpt.isPresent()) {
	        ...
	        // Also set it in the ticket if you're using bidirectional relation
